...
export default async function Home({
  ...
  const link = (path: string) => `${path}${path.includes("?") ? "&" : "?"}lang=${lang}`;
        ...
        <Card className="p-5">
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-sm text-black/70">
              {lang === "ar" ? "اختر قسم المنتجات" : "Choose a product section"}
            </p>
          </div>

          <div className="mt-5 grid gap-3">
            <Link href={link("/products")} className="no-underline">
              <Button className="w-full py-5 text-base">{labels.all}</Button>
            </Link>

            <Link href={link("/products?category=firewoods")} className="no-underline">
              <Button variant="outline" className="w-full py-5 text-base">{labels.fire}</Button>
            </Link>

            <Link href={link("/products?category=other")} className="no-underline">
