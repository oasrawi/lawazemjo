...
export default async function ProductsPage({
  ...
  searchParams: { lang?: string; category?: string; q?: string; inStock?: string };
}) {
  const settings = await getSettings();
  const lang = (searchParams.lang === "ar" ? "ar" : "en") as "en" | "ar";
  const category =
    searchParams.category === "firewoods" ? "firewoods" : searchParams.category === "other" ? "other" : undefined;
    ...
    <SiteShell logoUrl={settings.logoUrl} social={settings.social} theme={settings.theme}>
      <div className="grid gap-4">
        <div className="flex items-end justify-between gap-3">
          ...
          <Link href={withLang("/", lang)} className="text-sm font-semibold">{lang === "ar" ? "الرئيسية" : "Home"}</Link>
        </div>
