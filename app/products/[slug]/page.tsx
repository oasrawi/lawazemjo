import { SiteShell, withLang } from "@/components/SiteShell";
    ...
    <SiteShell logoUrl={settings.logoUrl} social={settings.social} theme={settings.theme}>
      <div className="grid gap-4">
        ...
        <Card className="overflow-hidden">
          <div className="aspect-[4/3] w-full bg-black/5 dark:bg-white/10">
            {p.images?.[0] ? (
          ...
          <div className="p-5">
            ...
            <p className="mt-2 text-sm font-semibold text-black/70 dark:text-white/70">{price} {lang === "ar" ? "دينار" : "JOD"}</p>
            {desc ? <p className="mt-3 text-sm text-black/70 dark:text-white/70 whitespace-pre-line">{desc}</p> : null}

            <div className="mt-5 grid grid-cols-2 gap-2">
              {/* client actions happen in client component */}
              <ClientActions
                lang={lang}
                slug={p.slug}
                title_en={p.title_en}
