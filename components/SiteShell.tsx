...
export function SiteShell({
  ...
  useEffect(() => {
    ...
    setLang(l);
    if (typeof window !== "undefined") window.localStorage.setItem("whatsapp_catalog_lang", l);
  }, []);
  ...
  const toggleLang = () => {
    ...
    setLang(next);
    if (typeof window !== "undefined") window.localStorage.setItem("whatsapp_catalog_lang", next);
    ...
    <div className="min-h-screen" dir={dir(lang)}>
      <header className="sticky top-0 z-20 border-b border-black/10 bg-white/80 backdrop-blur dark:border-white/10 dark:bg-black/60">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
          <Link href={withLang("/", lang)} className="no-underline">
          ...
          <div className="flex items-center gap-2">
            <Link href={withLang("/cart", lang)} className="no-underline">
...
function SocialIcon({ href, label }: { href: string; label: string }) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-black/10 bg-black/5 text-xs font-bold no-underline hover:bg-black/10 dark:border-white/15 dark:bg-white/10 dark:hover:bg-white/15"
      aria-label={label}
      title={label}
    >
      {label[0]}
