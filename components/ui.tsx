...
export function Button({
  ...
  const variants: Record<string, string> = {
    ...
    outline:
      "border border-black/10 bg-white text-black shadow-sm hover:bg-black/5 focus:ring-black/20 dark:border-white/15 dark:bg-white/5 dark:text-white dark:hover:bg-white/10",
    ghost: "bg-transparent text-black/80 hover:bg-black/5 focus:ring-black/20 dark:text-white/80 dark:hover:bg-white/10",
  };
  return (
...
export function Card({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "rounded-2xl border border-black/10 bg-white shadow-sm dark:border-white/10 dark:bg-white/5",
        className
      )}
      {...props}
    />
...
export function Input({ className, ...props }: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      className={cn(
        "w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none transition focus:ring-2 focus:ring-black/20 dark:border-white/10 dark:bg-white/5 dark:text-white dark:placeholder:text-white/50",
        className
      )}
      {...props}
    />
