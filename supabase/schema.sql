...
-- SETTINGS (logo + theme + social links)
...
create table if not exists public.analytics_events (
  id bigint generated by default as identity primary key,
  event text not null, -- 'page_view' | 'click' | 'add_to_cart' | 'order_now' | 'checkout'
  path text not null,
  lang text not null,
  product_slug text,
...
-- PUBLIC READ for settings/products
...
-- ADMIN write for settings/products: requires Supabase Auth user (you'll control via app logic + JWT)
-- Simplest: only authenticated users can write (you can later lock to specific users/roles).
...
-- Analytics insert allowed publicly (no auth) so you can track anonymously
...
-- Analytics read only for authenticated users (admin dashboard)
...
-- Storage: create a bucket named 'product-images' and 'branding' in Supabase Storage.
