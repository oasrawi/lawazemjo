...
export function whatsappMessageForSingle(lang: Lang, item: CartItem, notes?: string) {
  if (lang === "ar") {
    const lines = [
      ...
      `${item.title_ar} — ${formatJOD(item.price_jod)} دينار`,
    ];
...
export function whatsappMessageForCart(lang: Lang, items: CartItem[], notes?: string) {
  ...
  if (lang === "ar") {
    ...
    for (const it of items) {
      lines.push(`- ${it.title_ar} x${it.qty} — ${formatJOD(it.price_jod * it.qty)} دينار`);
    }
  ...
  for (const it of items) {
    lines.push(`- ${it.title_en} x${it.qty} — ${formatJOD(it.price_jod * it.qty)} JOD`);
  }
