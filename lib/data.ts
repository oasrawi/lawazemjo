...
export async function getSettings(): Promise<Settings> {
  ...
  return { logoUrl: branding.logoUrl ?? null, theme: branding.theme ?? null, social: social ?? null };
}
...
export async function listProducts(opts?: { category?: "firewoods" | "other"; q?: string; inStockOnly?: boolean }) {
  ...
  if (opts?.q) {
    query = query.or(`title_en.ilike.%${opts.q}%,title_ar.ilike.%${opts.q}%`);
  }
...
function normalizeProduct(row: any): Product {
  return { ...row, images: Array.isArray(row.images) ? row.images : [], price_jod: Number(row.price_jod ?? 0) } as Product;
}
